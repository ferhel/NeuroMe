...
def get_sequences_and_labels(words_id):
    ...
    if not words_id:
        print("Error: Lista de palabras vacía (words_id)")
        return [], []
    
    for word_index, word_id in enumerate(words_id):
        hdf_path = os.path.join(KEYPOINTS_PATH, f"{word_id}.h5")
        
        # 1. Verificar si el archivo HDF5 existe
        if not os.path.exists(hdf_path):
            print(f"Error: Archivo {hdf_path} no encontrado. Omitiendo.")
            continue
        
        try:
            # 2. Leer datos con manejo de errores
            data = pd.read_hdf(hdf_path, key='data')
            
            # 3. Verificar columnas requeridas
            if 'sample' not in data.columns or 'keypoints' not in data.columns:
                print(f"Error: Estructura inválida en {hdf_path}")
                continue
                
            # 4. Procesar muestras
            for sample_id, df_sample in data.groupby('sample'):
                # Convertir keypoints a numpy array explícitamente
                seq_keypoints = np.array([np.array(kp) for kp in df_sample['keypoints']])
                
                sequences.append(seq_keypoints)
                labels.append(word_index)
                
        except Exception as e:
            print(f"Error procesando {hdf_path}: {str(e)}")
            continue
